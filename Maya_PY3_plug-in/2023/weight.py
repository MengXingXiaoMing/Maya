...
class Weight:
            ...
            if source_skin_cluster:
                ...
                if copy_way == 'Normal':
                    ...
                    if target_type == 'Point':
                        ...
                        cmds.copySkinWeights(surfaceAssociation='closestPoint', normalize=1,
                                           influenceAssociation='oneToOne', noMirror=1)
                if copy_way == 'UV':
    ...
    def export_weight(self, Model):
        ...
        if not os.path.exists(path + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder'):
            os.makedirs(path + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder')
        # 清理与即将生成的文件重名的文件
        #print (MayaPath + '/scratch_file/MayaWeightExportImportWeightProvisionalFolder')
        if any(name.endswith(('.xml')) for name in
               os.listdir(MayaPath + '/scratch_file/MayaWeightExportImportWeightProvisionalFolder/')):
            my_path = (MayaPath + '/scratch_file/MayaWeightExportImportWeightProvisionalFolder/')
        ...
        for MD in Model:
            ...
            if SkinCluster:
                ...
                file = open((path + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder\\' + MD + '.txt'), "w")
                for Jon in Joint:
                    file.write(Jon + '\n')
                file.close()
                mel.eval('deformerWeights -export -deformer \"' + SkinCluster + '\" -path \"' + MayaPath + '/scratch_file/MayaWeightExportImportWeightProvisionalFolder/' + '\" \"' + MD + '.xml\";')
    ...
    def import_weight(self, Model):
        ...
        if not os.path.exists(path + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder'):
            print('\n没有找到本插件的权重存放文件夹，请先导出权重。\n如果要查询，下面是路径：' + '\n' + str(
                path) + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder\n')
        else:
            AllNodes = cmds.ls(type='joint')
            for MD in Model:
                if not os.path.exists(path + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder\\'+ MD + ".xml"):
                    print('\n如果没有找到本插件的权重存放文件夹，请先导出权重。\n如果要查询，下面是路径：' + '\n' + str(path) + '\scratch_file\MayaWeightExportImportWeightProvisionalFolder\\'+ str(MD) + ".xml"+'\n')
                else:

                    fo = open(path + "\scratch_file\MayaWeightExportImportWeightProvisionalFolder\\" + MD + ".txt", "r")
                    ...
                    cmds.deformerWeights((MD + ".xml"),
                                       path=(MayaPath + "/scratch_file/MayaWeightExportImportWeightProvisionalFolder/"), im=1,
