...
global proc getCurrentNodeInfo(string $nodeName) {
    ...
    string $cleanName = getCleanNodeName($nodeName);
    // 检查节点是否存在
    if (!`objExists $cleanName`) {
        warning("节点 " + $cleanName + " 不存在");
        print("================\\n");
        return;
    }
    ...
    // 检查节点类型
    string $nodeType = `nodeType $cleanName`;
    print("节点类型: " + $nodeType + "\\n");

    // 如果是变换节点，尝试获取其形状节点
    if ($nodeType == "transform") {
        string $shapes[] = `listRelatives -shapes $cleanName`;
        if (size($shapes) > 0) {
            $cleanName = $shapes[0];
            $nodeType = `nodeType $cleanName`;
            print("转换为形状节点: " + $cleanName + "\\n");
        }
    }
    ...
    if ($nodeType == "GetFromArray") {
        ...
        if (`attributeExists "textInput" $cleanName`) {
            string $textVal = `getAttr ($cleanName + ".textInput")`;
