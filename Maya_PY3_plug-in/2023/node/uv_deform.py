...
class UvDeform(ompx.MPxNode):
    ...
    def compute(self, plug, data_block):
        # 只处理 outMesh 插头
        if plug != UvDeform.outMesh:
            return

        # 1. 获取输入网格
        in_mesh_handle = data_block.inputValue(UvDeform.inMesh)
        ...
        # 2. 获取目标网格
        target_mesh_handle = data_block.inputValue(UvDeform.targetMesh)
        ...
        # 5. 设置输出
        output_handle = data_block.outputValue(UvDeform.outMesh)
    ...
    def process_uv_deform(self, in_mesh, target_mesh, out_mesh, data_block):
        ...
        # 获取权重值
        # input_handle = data_block.inputValue(UvDeform.input)
...
def initializePlugin(mobject):
    ...
    try:
        plugin.registerNode(
            UvDeform.node_name,
            UvDeform.Uv_id,
            UvDeform.nodeCreator,
            UvDeform.nodeInitializer
        )
    except:
        sys.stderr.write(f"注册节点失败: {UvDeform.node_name}")


def uninitializePlugin(mobject):
    plugin = ompx.MFnPlugin(mobject)
    plugin.deregisterNode(UvDeform.Uv_id)