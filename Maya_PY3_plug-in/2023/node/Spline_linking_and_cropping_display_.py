...
class SplineLinkingAndCroppingDisplay(ompx.MPxNode):
    ...
    # 更新ID防止冲突
    Uv_id = om.MTypeId(0x00141480 + 79)
    ...
    def compute(self, plug, data_block):
        ...
        try:
            ...
            sys.stderr.write("Failed to build sorted_indices: %s\n" % str(e))
            sorted_indices = []
        ...
        for i in sorted_indices:
            ...
            if input_data_obj.hasFn(om.MFn.kNurbsCurve):
                ...
                # 1.5. 获取该曲线的整体旋转角度
                current_curve_rotation = 0.0
                try:
                    curve_rotation_handle.jumpToElement(i)
                    current_curve_rotation = curve_rotation_handle.inputValue().asDouble()
                except Exception as e:
                    sys.stderr.write("Failed to read curve rotation[%d]: %s\n" % (i, str(e)))
                ...
                try:
                    ...
                    sys.stderr.write("Failed to read offset point[%d]: %s\n" % (i, str(e)))
                ...
                # 将 offset 和曲线旋转数据也存入元组中传递给下一个阶段
                curves_data.append((cvs, knots, cvs.length(), current_offsets, current_curve_rotation))
        ...
        combined_offsets = []           # 用于存储合并后的全局 per-CV offset
        combined_curve_rotations = []   # 用于存储每个 CV 对应的曲线级旋转
        ...
        for i, (cvs, knots, num_cvs, offsets, curve_rot) in enumerate(curves_data):
            # 合并 CV、Offset 和曲线旋转 (绝对的一一对应)
            for k in range(num_cvs):
                combined_cvs.append(cvs[k])
                combined_offsets.append(offsets[k])
                combined_curve_rotations.append(curve_rot)
        ...
        cropped_offsets = []          # 截取后的 per-CV offset
        cropped_curve_rotations = []  # 截取后的曲线级旋转
        for i in range(start_point_value, actual_end):
            cropped_cvs.append(combined_cvs[i])
            cropped_offsets.append(combined_offsets[i])
            cropped_curve_rotations.append(combined_curve_rotations[i])
        ...
        if plug == self.out_surface:
            ...
            for i in range(num_final_cvs):
                ...
                # 3. 计算最终旋转角度：全局旋转 + 曲线级旋转 + 该点专属偏移值
                # 三档叠加：rotation(全局) + curveRotation(每条曲线) + offsetValues(每CV)
                point_offset_angle = cropped_offsets[i]
                curve_rotation_angle = cropped_curve_rotations[i]
                total_angle_deg = base_rotation_angle + curve_rotation_angle + point_offset_angle
            ...
            try:
                ...
                sys.stderr.write("Failed to create complex surface: %s\n" % str(e))
        ...
        if plug.attribute() == self.in_curve:
            if plug.isElement():
                ...
                try:
                    ...
                    sys.stderr.write("connectionMade: Failed to set link_order[%d]: %s\n" % (index, str(e)))
                ...
                try:
                    ...
                    sys.stderr.write("connectionMade: Failed to set reverse_curve[%d]: %s\n" % (index, str(e)))
                    ...
                    if cv_count > 0:
                        ...
                        for j in range(cv_count):
                            try:
                                ...
                                sys.stderr.write("connectionMade: Failed to set offset_values[%d]: %s\n" % (j, str(e)))
                except Exception as e:
                    sys.stderr.write("connectionMade: Failed to init offset_point[%d]: %s\n" % (index, str(e)))

                # 初始化曲线级旋转角度为 0.0
                curve_rotation_plug = dep_node_fn.findPlug(self.curve_rotation, False)
                try:
                    target_element = curve_rotation_plug.elementByLogicalIndex(index)
                    target_element.setDouble(0.0)
                except Exception as e:
                    sys.stderr.write("connectionMade: Failed to set curve_rotation[%d]: %s\n" % (index, str(e)))
    ...
    def connectionBroken(self, plug, otherPlug, asSrc):
        if plug.attribute() == self.in_curve:
            if plug.isElement():
                ...
                # 清理 link_order：设为 -1 标记该位置已释放
                link_order_plug = dep_node_fn.findPlug(self.link_order, False)
                try:
                    target_element = link_order_plug.elementByLogicalIndex(index)
                    target_element.setInt(-1)
                except Exception as e:
                    sys.stderr.write("connectionBroken: Failed to clean link_order[%d]: %s\n" % (index, str(e)))

                # 清理 reverse_curve：重置为 0
                reverse_curve_plug = dep_node_fn.findPlug(self.reverse_curve, False)
                try:
                    target_element = reverse_curve_plug.elementByLogicalIndex(index)
                    target_element.setInt(0)
                except Exception as e:
                    sys.stderr.write("connectionBroken: Failed to clean reverse_curve[%d]: %s\n" % (index, str(e)))

                # 清理 offset_point：删除对应的复合子元素
                offset_point_plug = dep_node_fn.findPlug(self.offset_point, False)
                try:
                    offset_element = offset_point_plug.elementByLogicalIndex(index)
                    offset_values_plug = offset_element.child(self.offset_values)
                    # 将子数组清空，释放 CV 级偏移数据
                    existing_indices = om.MIntArray()
                    offset_values_plug.getExistingArrayAttributeIndices(existing_indices)
                    for j in range(existing_indices.length()):
                        cv_element = offset_values_plug.elementByLogicalIndex(existing_indices[j])
                        cv_element.setDouble(0.0)
                    offset_values_plug.setNumElements(0)
                except Exception as e:
                    sys.stderr.write("connectionBroken: Failed to clean offset_point[%d]: %s\n" % (index, str(e)))
                ...
                try:
                    ...
                    sys.stderr.write("connectionBroken: Failed to clean curve_rotation[%d]: %s\n" % (index, str(e)))
    ...
    def nodeInitializer(cls):
        ...
        # 每条曲线的整体旋转数组 (叠加到全局旋转之上)
        cls.curve_rotation = nAttr.create("curveRotation", "cr", om.MFnNumericData.kDouble, 0.0)
        nAttr.setArray(True)
        nAttr.setStorable(True)
        nAttr.setKeyable(True)
...
def initializePlugin(mobject):
    plugin = ompx.MFnPlugin(mobject, "KangmingZhan", "1.2.0")
    try:
        ...
        print("SplineLinkingAndCroppingDisplay节点注册成功！(含逐点旋转绑定)")
    except Exception as e:
