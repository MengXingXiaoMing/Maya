...
class SimpleSplineIKNode(om.MPxNode):
    ...
    def initialize():
        ...
        SimpleSplineIKNode.aSampleDensity = nAttr6.create("sampleDensity", "sden", om.MFnNumericData.kInt, 20)
        nAttr6.setMin(1)
        nAttr6.setMax(1000)
    ...
    def compute(self, plug, dataBlock):
        ...
        # 构建输入签名，检测是否与上次完全相同（跳过重复 compute）
        inputSig = (totalLength, jointCount, sampleDensity, uniformFallback, startOffsetU,
                    tuple(radiusValues))
        if plugAttr != self.aOutMatrices and inputSig == self._lastComputedData:
            dataBlock.setClean(plug)
            return
        self._lastComputedData = inputSig

        knots = curveFn.knots()
        uMin = float(knots[0])
        uMax = float(knots[-1])

        # 负偏移时削减半径：从 radius[0] 开始依次减去 |startOffset|
        if startOffsetU < 0.0 and radiusValues:
            remaining = abs(startOffsetU)
            for i in range(len(radiusValues)):
                if remaining <= 0.00001:
                    break
                if radiusValues[i] > remaining:
                    radiusValues[i] -= remaining
                    remaining = 0.0
                else:
                    remaining -= radiusValues[i]
                    radiusValues[i] = 0.0
        ...
        for i in range(1, actualJointCount):
            ...
            # 半径为 0 → 下一个骨骼留在同一位置，继续算后面的
            if r <= 0.0001:
                jointParams.append(prevParam)
                jointWorldPositions.append(prevPos)
                continue
        ...
        # 补齐剩余骨骼到曲线末端，防止未输出的元素保持 dirty 导致死循环卡顿
        while len(jointParams) < jointCount:
            jointParams.append(uMax)

        self._outputMatrices(plug, curveFn, curveWorldMat, upVec,
                             jointParams, jointCount, dataBlock)
    ...
    def _getJointCount(self, nodeObj):
        ...
        if curNum == self._cachedNumElements and self._cachedJointCount >= 0:
    ...
    def _outputMatrices(self, plug, curveFn, curveWorldMat, upVec,
                        jointParams, jointCount, dataBlock):
        """统一输出 + 全元素 setClean，确保输出 jointCount 个"""
        finalCount = jointCount
        ...
        for i in range(finalCount):
            param = jointParams[i] if i < len(jointParams) else jointParams[-1]
