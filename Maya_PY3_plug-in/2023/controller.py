...
class CurveControllerEdit:
        ...
        if cmds.objExists('All_ControllerSystem_Grp') and cmds.objExists('BaseRoot_Grp'):
            ...
            if cmds.objExists('MainSystem_Grp'):
                ...
                for ind in Independence_controller_sys:
                    ...
                    need_delete_object = need_delete_object + values_list[0]
                need_extract_grp = [x for x in all_child if x not in need_delete_object]
                if need_extract_grp:
                ...
                for dict in reduction_dict_list:
                    ...
                    child = dict['child']
                    if child:
                        child = [x for x in child if x not in need_extract_grp]
                ...
                if keep_curve_shape == True:
                    ...
                    for fk in FK_controller_sys:
                        ...
                        child = []
                        child = cmds.listRelatives(fk['FK_sys'][-1], c=True,type='transform')
                        cur = fk['FK_sys'][-1]
                        if child:
                            cmds.parent(child, parent)
                        cmds.parent(cur,w=1)
                    for fk in Independence_controller_sys:
                        ...
                        child = []
                        child = cmds.listRelatives(fk['Independence_sys'][-1], c=True,type='transform')
            ...
            for j in skin_joint:
                if cmds.objExists(j+'_IndependenceCurve'):
                    ...
                    self.modify_vontroller_shape("scale", 1, 1, 1)
                grp_str = ''
                for g in range(0,controller_group_num):
                    cmds.group(cur,n=(j+'_IndependenceGrp'+str(g+1)))
            ...
            for j in skin_joint:
                if cmds.objExists(j + '_FKCurve'):
                    ...
                    self.modify_vontroller_shape("scale", 2, 2, 2)
                grp_str = ''
                for g in range(0, controller_group_num):
                    cmds.group(cur, n=(j + '_FKGrp' + str(g + 1)))
    ...
    def create_controllers_system_copy_joint_part(self,other_suffix):
        ...
        for j in base_joint:
            ...
            if translocation[0] == 0:#如果对象在中间则判断其父对象是否在中间
                parent = cmds.listRelatives(j, p=1)
                translocation = cmds.xform(parent, q=1, ws=1, t=1)
                if translocation[0] == 0:#如果父对象还在中间则看看是否是顶组
        ...
        for j in middle_joint:
            child = cmds.listRelatives(j, c=1)
            if child:
                for c in child:
    ...
    def delete_addattr_system(self, attribute):
        ...
        if attribute == 'NoMirror':
            if not cmds.objExists(sel[0] + '.NoMirror'):
                cmds.addAttr(sel[0], ln='NoMirror', at='bool')
                cmds.setAttr((sel[0] + '.NoMirror'), e=1, keyable=True)
                cmds.setAttr((sel[0] + '.NoMirror'), 1)
            else:
                cmds.deleteAttr(sel[0], at='NoMirror')
        if attribute == 'aim':
