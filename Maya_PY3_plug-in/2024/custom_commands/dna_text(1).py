...
def Load_the_dictionary_to_be_modified_2(base_face):
    need_change_values = []
    getJointCount = reader.getJointCount() # 获取骨骼指数
    print('骨骼数量:' + str(getJointCount))
    getJointGroupCount = reader.getJointGroupCount() # 获取骨骼组指数
    ...
    print('需要修改的初始表情：' + str(base_face))
    print('基础表情名称:' + str(getRawControlName))
    getRawControlName = reader.getRawControlName(268)
    print('基础表情名称:' + str(getRawControlName))
    getRawControlName = reader.getRawControlName(269)
    print('基础表情名称:' + str(getRawControlName))

    # getJointParentIndex = reader.getJointParentIndex(0)
    # print('骨骼父对象指针:' + str(len(getJointParentIndex)) + str(getJointParentIndex))
    getBlendShapeChannelCount = reader.getBlendShapeChannelCount()
    print('获取bs系统数量:'+ str(getBlendShapeChannelCount))

    getBlendShapeChannelName = reader.getBlendShapeChannelName(base_face)
    print('获取bs系统名称:' + str(len(getBlendShapeChannelName)) + str(getBlendShapeChannelName))

    getBlendShapeChannelIndexListCount = reader.getBlendShapeChannelIndexListCount()
    print('获取bs系统指针列表数量:' + str(getBlendShapeChannelIndexListCount))

    getAnimatedMapCount = reader.getAnimatedMapCount()
    print('获取动画贴图数量:' + str(getAnimatedMapCount))

    for i in [0,1,2,3,4,5,6]:
        getAnimatedMapName = reader.getAnimatedMapName(i)
        print('获取动画贴图名称:' + str(getAnimatedMapName))

    getAnimatedMapIndexListCount = reader.getAnimatedMapIndexListCount()
    print('获取动画贴图指针列表数量:' + str(getAnimatedMapIndexListCount))
    ...
    print('微表情数量索引:' + str(len(getPSDRowIndices)) + str(getPSDRowIndices))
    getPSDValues = reader.getPSDValues()
    print('微表情数量索引:' + str(len(getPSDValues)) + str(getPSDValues))
    getJointRowCount = reader.getJointRowCount()
    print('微表情数量索引:'+ str(getJointRowCount))
    getJointColumnCount = reader.getJointColumnCount()
    print('微表情数量索引:' + str(getJointColumnCount))
    getJointVariableAttributeIndices = reader.getJointVariableAttributeIndices(0)
    print('微表情数量索引:' + str(len(getJointVariableAttributeIndices)) + str(getJointVariableAttributeIndices))
    ...
    print('需要修改的微表情:' + str(len(need_change_PSD_face)) + str(need_change_PSD_face))
    # 查询所有组中是否有要修改的主表情
    print('表情修改完毕')
#print_dna()
#modify_dna()
...
Load_the_dictionary_to_be_modified_2(191)
'''tempid = 123
print(tempid)
getJointCount = reader.getJointCount()
print('getJointCount:'+str(getJointCount))
