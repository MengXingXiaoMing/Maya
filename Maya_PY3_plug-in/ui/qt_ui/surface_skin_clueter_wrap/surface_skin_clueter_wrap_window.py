...
class Window(QtWidgets.QMainWindow):
    def __init__(self, parent=wrapInstance(int(Omui.MQtUtil.mainWindow()), QtWidgets.QWidget)):
        ...
        self.setWindowTitle('曲面蒙皮包裹(Maya'+self.maya_version+')')
        self.create_widgets()
        self.create_layouts()
        self.create_connect()
        ...
        self.ui_edit = UiEdit()
        self.weight = Weight()
        self.others_library1 = OthersLibrary()

        palette = self.palette()
        palette.setColor(QPalette.Window, QColor(37, 62, 113))
    ...
    def create_connect(self):
        ...
        self.button_13.clicked.connect(lambda: self.others_library1.switch_skin_model_hand_influence_state())  # 移动有权重的骨骼
    ...
    def reload_copy_source(self):
        self.ui_edit.load_select_for_ui_text(self.line_edit_1,['QLineEdit'])
        # print('加载拷贝源')
    ...
    def reload_copy_target(self):
        self.ui_edit.load_select_for_ui_text(self.line_edit_2, ['QLineEdit'])
        # print('加载拷贝目标')
    ...
    def copy_point_weight(self):
        ...
        target = target.split(',')
        if soure and target:
            self.weight.base_copy_joint_weight('Normal', soure, target, '', '')
        else:
    ...
    def unify_loop_edge_joint_weight(self):
        self.others_library.uniform_edge_loop_weights()
        # print('统一循环边权重')

    # 中线建立骨骼链
    def establishing_a_bone_chain_at_the_midline(self):
        self.others_library.establishing_a_bone_chain_at_the_midline()
        # print('中线建立骨骼链')
    ...
    def copy_weight(self):
        self.weight.copy_joint_weight()
        print('拷贝权重')

    # 对半拷贝权重
    def half_and_half_copy_weight(self):
        self.weight.half_and_half_copy_weight()
        print('对半拷贝权重')
    ...
    def automatically_adjust_the_slider_range_and_return_values_to_ui(self):
        ...
        num = self.automatically_adjust_the_slider_range_and_return_values_to_ui2(soure_ui, soure_ui_type)
    ...
    def modify_UI_values_and_provide_feedback_to_the_slider(self):
        ...
        self.ui_edit.give_the_value_to_the_slider(num, target_ui, target_ui_type)
    ...
    def smooth_joint_weight(self):
        ...
        self.weight.apply_smooth_weight(smooth_type, int(num))

        #self.smooth_joint_weight()
        #print('平滑权重')
    ...
    def reload_target_model(self):
        self.ui_edit.load_select_for_ui_text(self.line_edit_4, ['QLineEdit'])
        # print('加载目标模型')

    # 选择源模型合并权重到目标
    def select_source_model_merge_joint_weight_to_target(self):
        soure = self.line_edit_4.text()
        self.weight.select_source_model_merge_joint_weight_to_target(soure)
        #print('选择源模型合并权重到目标')

    # 导出权重
    def export_joint_weight(self):
        self.weight.export_weight(cmds.ls(sl=1))
        print('导出权重')

    # 导入权重
    def import_joint_weight(self):
        self.weight.import_weight(cmds.ls(sl=1))
        print('导入权重')
    ...
    def show_or_hide_bones(self):
        self.others_library.set_bone_display()
        print('显影骨骼')

    # 显影轴向
    def show_hide_axial(self):
        self.others_library.show_hide_axial()
        print('显影轴向')
    ...
    # 自动调整滑块范围且返回数值给ui
    def automatically_adjust_the_slider_range_and_return_values_to_ui2(self, soure_ui, soure_ui_type):
