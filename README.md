...
> 插件本体位于本仓库的 [`Maya_PY3_plug-in/`](Maya_PY3_plug-in) 目录，本页即该插件的完整说明。
...
1. **改版本文件夹名**
   把插件目录下的版本文件夹改成你正在使用的 Maya 版本号（`Maya_PY3_plug-in/2023` → 例如改名为 `2024`）。
   插件在加载时会从当前 Maya 版本号向下查找第一个存在的版本文件夹，所以文件夹名不低于你的 Maya 版本即可正常加载。

2. **执行安装脚本**
   把 `Maya_PY3_plug-in/ui/Install.py` 直接拖拽到 Maya 视窗中运行，弹出安装窗口。
...
核心模块一览（`Maya_PY3_plug-in/2023/`，随版本改名）：
...
## 内置工具（Maya_PY3_plug-in/ui/qt_ui）
...
## 自定义节点（Maya_PY3_plug-in/2023/node）
...
其中**魔方绑定节点**已单独整理文档，见 [Maya_PY3_plug-in/2023/node/README.md](Maya_PY3_plug-in/2023/node/README.md)：支持自定义长宽高（如 4×4×4）、每层独立控制器与总控控制器。
...
- `Maya_PY3_plug-in/.gitignore` 已排除 `_*.py` / `_*.log` 调试脚本、`__pycache__`、`*.pyc`、`.idea`、`Thumbs.db` 以及运行时的 `scratch_file` 暂存目录。
