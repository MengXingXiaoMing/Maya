...
class ZKM_PresetTemplateWindowClass:
    def __init__(self):
    ...
    def ZKM_Window(self):
        ...
        pm.button(c='detectionOfTheSameNameMain()', bgc=(0, 0.8, 0.8), l="去除重复名称节点")
        pm.setParent('..')
        ...
        pm.formLayout('WindowControllerProcessingAllControllerFormLayout',h=300)
        ...
        WindowControllerProcessingAllControllerSliderLineMax = (len(AllTxtFile)/5/2*74-300)/10
        ...
        pm.intSlider('WindowControllerProcessingChangeCurveColor_intSlider',max=31, step=1, value=17, min=0,w=225,cc='DragSliderAutomaticallyModifyColorPanel()')
        ...
        pm.button(c='ZKM_CurveControllerEditClass().ZKM_ControllerHoming()', bgc=(1, 1, 1), l="归位")
        pm.button('CHUANGJIANFK', c='ChuangJianFK()', bgc=(1, 1, 1), l="创建控制器")
        pm.setParent('..')
        pm.setParent('..')
        ...
        pm.button(c='HairFollicleAdsorption()', l="毛囊附着(选择需要跟随的物体)")
        pm.setParent('..')
        pm.setParent('..')
        ...
        pm.showWindow()
ShowWindow = ZKM_PresetTemplateWindowClass()
# 生成控制器并刷新窗口
...
# 去除重复命名物体
def detectionOfTheSameNameMain():
    ZKM_RootDirectory = os.path.join('/'.join(os.path.abspath(inspect.getsourcefile(lambda: 0)).split('\\')[:-4]))
