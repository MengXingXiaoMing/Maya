...
class ZKM_QvanZhongChuLiWindowClass:
    ...
    def ZKM_Window(self):
        ...
        pm.setParent('..')
        pm.text(l='模型提交前可能需要处理部分')
        pm.button(c='ZKM_JointWeightProcessingClass().NormalizeWeight(pm.ls(sl=1),1)', l='为当前选择模型修复模型稍微移动一点就抖动的情况(归一化权重)')
        pm.rowColumnLayout(adj=1, nc=2)
        pm.button(c='ZKM_JointWeightProcessingClass().HandlingWeightJitter(pm.ls(sl=1),1)', bgc=(1,1,1),
                  l='为当前选择骨骼修复在低版本情况下模型移动过远点出现抖动情况')
        pm.button(c='ZKM_JointWeightProcessingClass().HandlingWeightJitter(pm.ls(sl=1),0)', bgc=(0.7,0.7,0.7),
                  l='选择主骨骼撤回当前修复')
        pm.setParent('..')
        pm.setParent('..')
        pm.showWindow()
