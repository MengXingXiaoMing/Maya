...
class ZKM_AddintermediateWindow():
    def ZKM_Window(self):#窗口
        if cmds.window('AddintermediateWindow', ex=1,cc='CleanWindow()'):
            cmds.deleteUI('AddintermediateWindow')
        cmds.window('AddintermediateWindow', t="按规范自动创建中间帧")
        cmds.rowColumnLayout(nc=1, adj=5)
        cmds.textFieldButtonGrp('LoadNameA',bl='加载源（组或者单个模型）',bc='ZKM_AddintermediateWindow().ZKM_LoadText(\'textFieldButtonGrp\' , \'LoadNameA\')')
        cmds.textFieldButtonGrp('LoadNameB',bl='加载BS名称',bc='ZKM_AddintermediateWindow().ZKM_LoadText(\'textFieldButtonGrp\', \'LoadNameB\')')
        cmds.textFieldButtonGrp('LoadNameC',bl='填写除数字外区别',bc='ZKM_AddintermediateWindow().ZKM_LoadText(\'textFieldButtonGrp\', \'LoadNameC\')')
        cmds.button(l='开始生成', command='ZKM_AddintermediateWindow().Create()')
        cmds.showWindow()
    def Create(self):
        ...
        self.ZKM_AsSpecificationsAddBSIntermediateFrame(B, A, C)
